	...
	if (existsConversation.length === 0) {
		...
		if (conversationId) {
			...
			const transferId = await insert(
				"INSERT INTO transfers(amount, currency, state, reference) VALUES(?, ?, ?, ?)",
				[25000, "DKK", "initiated", "seed-transfer-1"]
			);
			await execute(
				"INSERT INTO chat_items(conversation_id, type, sender_type, sender_id, transfer_id) VALUES(?, ?, ?, ?, ?)",
				[conversationId, "transfer", "company", 1, transferId]
			);
		}
	}
};
